...
class Solution(object):
    def rootSum(self, root, sum):
            ...
            if sum == root.val:
                res = 1
            return res + self.rootSum(root.left, sum-root.val) + self.rootSum(root.right, sum-root.val)
    ...
    def pathSum(self, root, targetSum):
        ...
        if not root:
            return 0
        return self.pathSum(root.left, targetSum)+self.pathSum(root.right, targetSum)+self.rootSum(root, targetSum)
