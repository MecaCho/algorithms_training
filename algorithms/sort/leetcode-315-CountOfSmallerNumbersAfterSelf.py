...
class Node():
    ...
    def add(self, root, node, res, index):
        if not root:
            return node
        if root.val >= node.val:
            root.count += 1
            root.left = self.add(root.left, node, res, index)
        else:
            res[index] += root.count + 1
            root.right = self.add(root.right, node, res, index)
        return root
...
但这样带来的算法效率会非常的低，原因在于每次寻找比num[i]小的数都需要从末尾遍历一次，所以时间复杂度为O(n^2)O(n 
2
 )
...
left、right两个区间中满足条件的对数已经在上一次二分中算出
每次二分只需要算出left 和 right 之间满足条件的对数即可
...
我们只需要遍历right中的数并在left区间找到对应范围，这个范围只需要满足第二个条件**nums[i]>nums[j]**即可，因为第一个条件是一定满足了的。(i<mid<j)

步骤：

利用归并排序递归求解两个子区间内部对数并进行归并排序，此时left和right为递增区间
排序的同时遍历 在right区间中的数(设为nums[i])，left区间找到满足这个条件的范围findl~findr,findr~findl即为左区间和 **nums[i]**匹配的点个数。
...
树状数组中*getSum(index)*作用是求原始数组nums中小于或等于当前index的数的和，此处用来统计逆序数，可以把getSum(x)看作是nums中小于x的数的个数的和。
