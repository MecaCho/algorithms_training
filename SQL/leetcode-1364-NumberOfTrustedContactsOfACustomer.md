...
```
SELECT i.invoice_id, customers.customer_name, i.price, COUNT(contacts.user_id) AS contacts_cnt, COUNT(c2.email) AS trusted_contacts_cnt
FROM Invoices i
INNER JOIN Customers customers ON i.user_id = customers.customer_id
...
GROUP BY i.invoice_id order by i.invoice_id;
